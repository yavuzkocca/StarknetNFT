...
export function processTimeLimitSet(
  event: CreatedEditionEvent,
  timeLimitSet: ethereum.Log
): void {
  ...
  // no 0x prefix
  let collectionAddressStr = timeLimitSet.data
    .toHexString()
    .slice(2 + 24, 2 + 64)
    .toLowerCase();
  if (
    !collectionAddress
      .toHexString()
      .toLowerCase()
      .includes(collectionAddressStr)
  ) {
    log.warning(
      'In tx {}, collection address {} in TimeLimitSetEvent does not match editionContractAddress {} in CreatedEditionEvent',
      [
        event.transaction.hash.toHexString(),
        collectionAddressStr,
  ...
  let deadlineHexStr = timeLimitSet.data
    .toHexString()
    .slice(2 + 64, 2 + 128);
  let deadlineBytes = Bytes.fromHexString(deadlineHexStr);
  let deadlineBytesLittleEndian = Bytes.fromUint8Array(
    deadlineBytes.reverse()
  );
  entity.deadline = BigInt.fromUnsignedBytes(
    deadlineBytesLittleEndian
  );
...
export default function handleCreatedEdition(
  event: CreatedEditionEvent
): void {
  let receipt = event.receipt;
  if (!receipt) {
    log.critical('No receipt for event in tx {}', [
  ...
  for (let i = 0; i < receipt.logs.length; i++) {
    ...
    if (
      topic.includes(
        '0x776e7116fa43d9385f3fd667b2e34d792fd3eec4b5a0aa8d1758b877ab4d6cbe'
      )
    ) {
